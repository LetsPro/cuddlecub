...
export function AdmissionsPage() {
  ...
  async function loadAdmissions() {
    ...
    try {
      const [inquiryResponse, admissionResponse, studentResponse] = await Promise.all([
  ...
  async function handleInquirySubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      closeInquiryModal();
      setMessage(editingInquiryId ? 'Inquiry updated.' : 'Inquiry added.');
    } catch (error) {
      setMessage(getErrorMessage(error));
    }
  }
  ...
  async function handleAdmissionSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await loadAdmissions();
      closeAdmissionModal();
      setMessage(editingAdmissionId ? 'Admission record updated.' : 'Admission workflow entry created.');
    } catch (error) {
      setMessage(getErrorMessage(error));
    }
  }
  ...
  return (
    <div className="space-y-6">
          ...
          columns={[
            {
              ...
              render: (row) => (
                <div>
                  <p className="font-bold text-slate-900">{row.child_name}</p>
                  <p className="text-xs text-slate-500">{row.parent_name}</p>
                </div>
              ),
            },
      ...
      <SectionCard
        ...
        description="See linked students, inquiry source, waitlist position, and final enrollment status from one tracker."
        action={
          <div className="flex flex-wrap justify-end gap-2">
          ...
          columns={[
            {
              ...
              render: (row) => {
                ...
                return (
                  <div>
                    <p className="font-bold text-slate-900">{linkedStudent || linkedInquiry?.child_name || 'Direct admission entry'}</p>
                    <p className="text-xs text-slate-500">{linkedInquiry ? linkedInquiry.parent_name : 'Walk-in / direct record'}</p>
                  </div>
                );
              },
            },
            {
              ...
              render: (row) => (row.student_id ? studentLookup[row.student_id] ?? 'Linked student' : 'Not linked yet'),
            },
            {
              key: 'inquiry',
              label: 'Inquiry',
              render: (row) => {
                const linkedInquiry = row.inquiry_id ? inquiryLookup[row.inquiry_id] : null;
                return linkedInquiry ? linkedInquiry.child_name : 'Walk-in / direct';
              },
            },
            { key: 'status', label: 'Status', render: (row) => <StatusBadge value={row.status} /> },
      ...
      <Modal
        description="Capture the child, parent, lead source, and current pipeline status in a clean inquiry record."
      ...
      <Modal
        description="Track document collection, onboarding progress, waitlist rank, and final enrollment status in one admission record."
      ...
      >
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleAdmissionSubmit}>
          <div>
            ...
            <select className="form-input" onChange={(event) => setAdmissionForm((current) => ({ ...current, inquiry_id: event.target.value }))} value={admissionForm.inquiry_id}>
          ...
          <div>
            ...
            <select className="form-input" onChange={(event) => setAdmissionForm((current) => ({ ...current, student_id: event.target.value }))} value={admissionForm.student_id}>
